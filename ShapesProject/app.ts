...
if (shape != 1 && shape != 2 && shape != 3) {
    ...
    switch (shape) {
        case 1: //Square
            ...
            if (shapeWidth < 1 || isNaN(shapeWidth)) //Verying input 
            {
        ...
        case 2: //Rectangle
            ...
            if (!isError) {
                ...
                if (shapeLength < 1 || isNaN(shapeLength)) //Verying input 
                {
                    alert("Error. Please enter a positive integer number.");
                    isError = true;
                }
    ...
    if (!isError) {
        ...
        let shapeForm: number = Number(prompt("Please select shape content:\n 1) * * * * *\n 2) *         *\n 3) 1 2 3 4 5\n 4) 5 4 3 2 1"));
        ...
        if (!isError) {
            switch (shape) {
                case 1: printRectangle(shapeWidth, shapeWidth, shapeForm); break; //Calling the function that prints the rectangle, using same input for width and for length so it will print a square
                case 2: printRectangle(shapeWidth, shapeLength, shapeForm); break; //Calling the function that prints the rectangle
                case 3: printTriangle(shapeWidth, shapeForm); break; //Calling the function that prints the triangle
            }
        }
    }
    //Function that prints out rectangle (including square)
    function printRectangle(rows: number, columns: number, form: number): void {
        let index: number;
        let index2: number;
    ...
    function printTriangle(width: number, form: number): void {
        let index: number;
        let index2: number;
        ...
        let tempperimetere = width + width + Math.sqrt(2 * width * width)
        tempperimetere *= 100;
